...
enum Scene { MENU, START_COUNTDOWN, GAME, END };
...
unsigned int VBO, VAO, EBO, texture, texturaTitulo, texturaMensaje;
...
for (int i = 0; i < 5; ++i) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        switch (currentScene) {
           ...
           case GAME: {glClearColor(0.1f, 0.1f, 0.15f, 1.0f);
                        glClear(GL_COLOR_BUFFER_BIT);
                        ...
                        if (isCountingDown) {
                            ...
                            // Juego normal después de la cuenta
...
void processInput(GLFWwindow* window) {
    ...
    if (glfwGetKey(window, GLFW_KEY_ENTER) == GLFW_PRESS && !enterPressed) {
        if (currentScene == MENU) currentScene = GAME;
        else if (currentScene == GAME) currentScene = END;
        else if (currentScene == END) currentScene = MENU;
        enterPressed = true;
    }
